...
def create_hospital_type_bar_chart(file_path, chart_title="Utilization Rates by Hospital Type"):
    ...
    df_hosp.columns = ["Hospital Type", "Total Enrollees", "Total With Utilization"]

    # Convert numeric columns, handling errors
    df_hosp["Total Enrollees"] = pd.to_numeric(df_hosp["Total Enrollees"].str.replace('.', '', regex=False), errors='coerce')
    df_hosp["Total With Utilization"] = pd.to_numeric(df_hosp["Total With Utilization"].str.replace('.', '', regex=False), errors='coerce')

    # Drop rows with missing values
    df_hosp = df_hosp.dropna(subset=["Total Enrollees", "Total With Utilization"])

    # Calculate utilization percentage
    df_hosp["Utilization Rate (%)"] = (df_hosp["Total With Utilization"] / df_hosp["Total Enrollees"]) * 100
