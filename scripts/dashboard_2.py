...
def create_strip_plot_dashboard(file_path, title="Program Payments by Demographic Group"):
    ...
    df = df[~df.iloc[:, 0].isin(["Total", "Unknown"])]  # Assuming first column is "Demographic Group"

    # Rename only relevant columns
    df.rename(columns={
        df.columns[0]: "Demographic Group",
        df.columns[5]: "Total Program Payments"
    }, inplace=True)
    ...
    category_map = {
        "Under 65 Years": "Age", "65 Years and Over": "Age",
