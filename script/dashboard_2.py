...
def create_strip_plot_dashboard(file_path, title="Program Payments by Demographic Group"):
    ...
    category_orders={"Main Category": ["Age", "Sex", "Race", "MME Status"]},
    opacity=0.6,
    hover_data={"Demographic Group": True, "Total Program Payments": True}
)

    fig3.update_traces(marker=dict(symbol="circle", size=6))
    fig3.update_layout(
    xaxis_title="Demographic Group", 
    yaxis_title="Program Payments ($)", 
    xaxis_tickangle=45,
    showlegend=False,  
    template="plotly"  # Set the theme
)
